...
function adjacent(grid, num, gridX) {
	var iterations = grid.length/gridX;
	var max = 0;
	for (i=0;i<iterations;i++) {
		for (x=0;x<gridX-num;x++) {
			var total = 1
			for (y=0;y<num;y++) {
...
function diagonalRight(grid, num, gridX) {
	var iterations = grid.length/gridX;
	var max = 0;
	for (i=0;i<iterations-num;i++) {
		for (x=0;x<gridX-num;x++) {
			var total = 1
			for (y=0;y<num;y++) {
...
function diagonalLeft(grid, num, gridX) {
	var array = []
	var iterations = grid.length/gridX;
	for (i=0;i<iterations;i++) {
	...
	return diagonalRight(array, num, gridX);
}

function down(grid, num, gridX) {
	var iterations = grid.length/gridX;
	var max = 0;
	for (i=0;i<(iterations-num);i++){
		for (y=0; y<gridX;y++){
...
function combineDirections(grid, num, gridX) {
	var max = 0;
	if (adjacent(grid,num,gridX) > max){
		max = adjacent(grid,num,gridX);
	}
	if (diagonalRight(grid,num,gridX) > max){
		max = diagonalRight(grid,num,gridX);
	}
	if (diagonalLeft(grid,num,gridX) > max){
		max = diagonalLeft(grid,num,gridX);
	}
	if (down(grid,num,gridX) > max){
		max = down(grid,num,gridX);
	}
	return max;
}

console.log(combineDirections)
